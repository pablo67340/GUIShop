...
public class PlaceholderMessage extends Message {
    ...
    private void generateMessage() {
        ...
        if (placeholders == null || placeholders.length == 0) message = null;
        else message = split(messageStr).toArray(new Object[0]);
    }

    /**
     * Splits an input in an List of Objects: integers for the index of the placeholders, Strings for other parts of the message.
     *
    ...
    /**
     * Tries to translate this message with given parameters.
     * All parameters will be converted to strings. The order is specified in the default messages.yml
     *
    ...
    public String translate(@Nullable Object... params) {
        ...
        for (Object obj : message) {
            if (obj instanceof String) output.append(obj);
            else {
                if ((int) obj >= params.length) output.append("%" + placeholders[(int) obj] + "%");
                else output.append(params[(int) obj]);
            }
        }
